...
{
    public class OfertaInmobiliariaRepository : IOfertaInmobiliariaRepository
    {
        private readonly IConnectionFactory _connectionFactory;

        public OfertaInmobiliariaRepository(IConnectionFactory connectionFactory)
        {
        ...
        public async Task<IEnumerable<OfertaInmobiliariaEntity>> GetList()
        {
            using (var conexion = _connectionFactory?.GetConnection())
            {
                ...
                return await conexion.QueryAsync<OfertaInmobiliariaEntity>("PKGSTP_GARANTIA_CS_JOSE.SPRNSTP_PORTALFMV3", param: dynamicParameters, commandType: CommandType.StoredProcedure);
            }
        }
    }
}
