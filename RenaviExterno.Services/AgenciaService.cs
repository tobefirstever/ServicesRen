...
namespace RenaviExterno.Services
{
    public class AgenciaService : IAgenciaService
    {
        ...
        {
            var model = new Response<List<AgenciaResponseDto>> { Data = new List<AgenciaResponseDto>() };

            try
            {

                var APIURL = ConfigurationManager.AppSettings?["UrlService"]?.ToString();
                var url = $"{APIURL}/api/agencia";
                ...
                var datos = JsonConvert.DeserializeObject< List<AgenciaResponseDto>> (result);
                model.Data = datos;
